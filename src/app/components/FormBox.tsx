...
export default function FormBox() {
    ...
    const connectCreateDB = async ({
        host,
        port,
        user,
        password,
        dbName,
    }: ConnectionParams): Promise<QueryResult> => {
        try {
    ...
    const updateCredentials = async ({
        ...
        password,
        email,
    }: DBCredentialsParams): Promise<QueryResult> => {
        try {
    ...
    const updateSiteInformation = async ({
        ...
        appName,
        aboutApp,
    }: UpdateSiteParams): Promise<QueryResult> => {
        try {
    ...
    const uploadPfpImages = async ({
        sitePfp,
        userPfp,
    }: UploadPfpImageParams): Promise<QueryResult> => {
    ...
    const createTable = async ({
        tableName,
        columns,
    }: CreateTableParams): Promise<QueryResult> => {
        try {
    ...
    const finalInstallCheck =
        async (): Promise<FinalInstallCheckQueryResult> => {
            try {
                const response = await fetch(
                    "/api/connections/finalInstallCheck",
                    {
                        method: "GET",
                    }
                );
                if (!response.ok) {
                    throw new Error(
                        `API error (Check SQL Server Status): ${response.statusText}`
                    );
                }
                const data = await response.json();
                return data;
