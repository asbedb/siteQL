...
export default function InstallationProgress() {
        ...
        const fetchInstallationData = async () => {
            try {
                const response = await fetch(
                    "/api/connections/checkInstallation"
                );
                if (!response.ok) {
    ...
    return (
        <div className="p-2">
            ...
            <p className="font-bold text-medium my-2 ">
                {installationData.message}
            </p>

            {installationData.status === "pending" ? (
                <div>
                    <ul className="px-4 py-2 bg-primary-100 rounded-xl">
                        {installationData.data.pendingInstallation.map(
                            (item, index) => (
                ...
                <div>
                    <ul className="px-4 py-2 bg-primary-100 rounded-xl">
                        {installationData.data.installedVariables.map(
                            (item, index) => (
                                <li key={index}>{item}</li>
                            )
                        )}
